...
export const getQuests = (quests = []) => quests.map((quest) => `<q lblue>Квест</q>: ${quest}`).join("<br/>");
			...
			if (allbossses.hasOwnProperty(key)) {
				...
				for (let i = 0; i < bosses.length; i++) {
					if (currBoss.trim() === bosses[i].na.trim()) {
						if (bosses[i].lvl) strBoss += ` <q lblue>${bosses[i].lvl}</q><q tsm llblue>lvl</q>`;
						if (modeModal) {
							if (bosses[i].xp) strBoss += ` <q lblue>${bosses[i].xp}</q><q tsm llblue>xp</q>`;
							if (bosses[i].hp) strBoss += ` <q lred>${bosses[i].hp}</q><q tsm llred>hp</q>`;
